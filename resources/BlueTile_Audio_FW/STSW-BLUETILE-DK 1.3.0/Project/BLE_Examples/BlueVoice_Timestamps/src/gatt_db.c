...
tBleStatus Add_HW_SW_ServW2ST_Service(void) {
	...
	ret = aci_gatt_add_service(UUID_TYPE_128, &service_uuid, PRIMARY_SERVICE, Services_Max_Attribute_Records[HW_SERVICE_INDEX] + 2, &HWServW2STHandle);
	...
	addTimestampCharacteristic(HWServW2STHandle);
...
/*
		...
		*/
		...
		uint8_t out_buff[5];
		for(int i=0; i<data_length || i<5; i++) {
			received_value += att_data[i] << 8*i;
		}
		received_value++;
		uint32_t currentTimestamp = getTimestamp();
		out_buff[0] = currentTimestamp;
		out_buff[1] = currentTimestamp >> 8;
		for(int i=0; i<data_length || i<5; i++) {
			out_buff[i+2] = received_value >> 8*i;
		}
		aci_gatt_update_char_value(ConfigServW2STHandle, ConfigCharHandle, 0, 7, out_buff); 

...
def update_time_sync():
    global time_sync_var, time_sync_handles, devices, time_sync_done
...
class MyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        global time_sync_var, time_sync_done
        ...
        # IMU notifications
        if cHandle == self.imu_handle and time_sync_done:
        ...
        # ENV notifications
        if cHandle == self.env_handle and time_sync_done:
        ...
        if cHandle == self.bv_sync_handle and time_sync_done:
            process_sync_variables(data)
        if cHandle == self.bv_aud_handle and time_sync_done:
            process_audio(data, self.audio_file)
        if cHandle == self.bv_time_handle and time_sync_done:
